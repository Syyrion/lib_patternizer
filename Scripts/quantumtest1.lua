...
function qSpiral(times, delayMult, position, direction, width, gap, step, spacing)
	local seamless = false --true
	if gap == step and width == step or gap == 1 then --this might need changing with step being a list EDIT: it does
		delayMult = 1
		seamless = false
	end
	...
	local stepSum = 0
	for x = 1, #step do 
		stepSum = stepSum + step[x]
	end
	local stepMult = stepSum
	stepMult = stepMult/#step
	
	stepSum = stepSum * math.floor(times/#step)
	if times % #step ~= 0 then
		for y = 1, (times % #step) + 1 do
			stepSum = stepSum + step[y]
		end
	end

	local idealDEL, sides = getDelayAndSides()
	local minDEL = (stepSum/stepMult - gap)/(times -  1) * idealDEL * 1.01 + pulseContrib()
	local resultDEL = lerp(minDEL, idealDEL, delayMult)

	local j = 1
	
	local thickness = 4*trueSpeedMult() * resultDEL * (seamless and 1.1 or 1) * stepMult
	local k = 0
	local createSpiral = function (p, i)
		w_wall(p + i, thickness)
	end
